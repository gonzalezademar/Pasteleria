  ...
  useEffect(() => {
    (async () => {
      try {
        setLoading(true);
        setErrorStatus(null);
        // Por ahora cargamos todo, luego filtraremos por el shop_id asociado al slug
        const [cats, prods] = await Promise.all([getCategories(), getProducts()]);
        
        if (cats.length === 0 && prods.length === 0) {
          setErrorStatus("No se encontraron datos para esta pastelería.");
        }
    ...
    <main className="container" style={{ paddingTop: '100px', paddingBottom: '80px', minHeight: '100vh' }}>
      ...
      <header style={{ textAlign: 'center', marginBottom: '60px' }}>
        ...
        <p style={{ color: 'var(--text-muted)', maxWidth: '600px', margin: '0 auto', marginBottom: '40px' }}>
          ...
          {slug && <span style={{ display: 'block', fontSize: '0.8rem', marginTop: '10px', opacity: 0.5 }}>Tienda: {slug}</span>}
        </p>
